...
### ATS recruiter intake persistence (Prisma)

The backend persists recruiter-created candidates and CV metadata in PostgreSQL via Prisma using:

| Table | Purpose |
| --- | --- |
| `Candidate` | Core candidate profile (name, email, optional phone/address/education/workExperience text, `createdByUserId` FK to `User`, timestamps). Indexed on `email`. |
| `CandidateDocument` | CV metadata (`documentType` = `CV`): original file name, MIME type, byte size, `storageKey` for secure/local storage path; FK to `Candidate` with cascade delete. |

Validation rules for this flow are enforced in the application layer (required fields, email format, E.164-compatible phone, length limits). Files are restricted to PDF and DOCX with a configurable maximum size (`CV_MAX_SIZE_MB`, default 5 MB).
